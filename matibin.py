...
R = float(input("Enter the reflux ratio R(enter inf for infinite reflux(R = inf))\nR = "))
if R > 99999:
    R = 99999
    q = 1.01 # just a random num, cause it's not important in such case
else:
    q = float(input("Enter the value of q, where q = (C_pm(t_BP - t_F))/r_m + 1\nq = "))
x_w = float(input("Enter the Bottoms composition(x_W), IN MOLE FRACTION!!!\nx_W = "))
x_d = float(input("Enter the Distillate composition(x_W), IN MOLE FRACTION!!!\nx_D = "))
x_f = float(input("Enter the Feed composition(x_W), IN MOLE FRACTION!!!\nx_F = "))
delta_k = q / (q - 1)
delta_b = x_f / (q - 1)
